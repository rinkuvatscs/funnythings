...
public class InterviewServiceImpl implements InterviewService {
	...
	public String addInterview(UserDetail userDetail, MultipartFile file) {
		...
		if (!StringUtils.isEmpty(addUserDetail)
				&& addUserDetail.getUserId() != 0) {

			if (!StringUtils.isEmpty(file)) {
				boolean isFileSaved = FileProcessingUtil.fileSaved(file,
						fileLocation);
				if (isFileSaved) {
					...
					try {
						int response = jdbcTemplate.update(
								QueryConstants.INTERVIEW_DETAIL_ADD, args.toArray());
						if (response != 0) {
							message = INTERVIEW_SAVED + userDetail.getUserId();
						} else {
							message = INTERVIEW_NOT_SAVED
									+ userDetail.getUserId();
						}
					} catch (Exception e) {
						e.printStackTrace();
						message = INTERVIEW_NOT_SAVE_ERROR
								+ userDetail.getUserId();
					}
